...
### Added
- **Drone & Raster Integration (Silvi Phase 3)**: Added `059_drone_raster_integration.sql` with MSAVI column on remote_sensing_observation, 3 new governed tables (raster_metadata for GeoTIFF/orthomosaic references, spatial_cluster for DBSCAN results, pest_hotspot for spatial pest clustering), 3 public views (v_public_spatial_clusters, v_public_pest_hotspots, v_public_canopy_analysis), `services.analytics.spatial_analytics` (4 functions: compute_spatial_clusters, compute_pest_hotspots, compute_canopy_analysis, compute_gap_detection), Adelphi pilot data (2 raster metadata records, 3 spatial clusters, 1 pest hotspot), and 16 tests.
