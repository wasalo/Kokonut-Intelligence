...
### Added
- **CRISP Risk Scoring Engine**: Added `076_crisp_risk_scoring.sql` with 8 new governed tables (crisp_risk_dimension for dimension definitions with configurable weights, crisp_location_weight for per-location weight overrides, crisp_risk_assessment for master assessment per location per period, crisp_carbon_yield_risk for carbon yield scenario detail, crisp_climate_risk for climate hazard scoring, crisp_policy_risk for policy & legal risk, crisp_financial_risk for financial viability, crisp_implementation_risk for implementation/developer risk), 2 views (v_crisp_composite_rating for published assessments, v_crisp_latest_assessment for latest per location), `services/crisp/` package with 11 modules (config, models, normalization, carbon_yield, climate_risk, policy_risk, financial_risk, implementation_risk, scoring_engine, cli, __main__), five-factor composite scoring (carbon yield 40%, climate 25%, policy 15%, financial 10%, implementation 10%) with configurable per-location weights and AAA-D rating scale, adapted from Solid World's SW-CRISP framework. Carbon yield risk uses 3-scenario modeling (minimum/realistic/optimistic) with NDVI-based vegetation density. Climate risk scores 6 hazards (drought, flood, heatwave, fire, storm, water stress) on 0-3 scale with mitigation factor. Policy risk assesses certification, carbon rights, land tenure, community alignment. Financial risk combines revenue, cost, liquidity, and market price factors. Implementation risk evaluates track record, team strength, network, transparency. CLI with 8 analysis flags (--carbon-yield, --climate, --policy, --financial, --implementation, --composite, --rate, --weights). Adelphi pilot data (5 dimension definitions, 5 farm impact mappings). 55+ unit tests covering normalization, scenarios, hazard scoring, sub-factors, and rating bands. Full methodology documentation at `docs/crisp-risk-scoring.md`.
- **Telemetry Infrastructure: Data Freshness + ClickHouse Sync + Climate Data**: Added `077_telemetry_infrastructure.sql` with 4 new governed tables (data_freshness_config for SLA definitions per source, data_freshness_check for freshness check results with status/alert tracking, remote_sensing_job for automated RS fetch configurations with provider/cadence/bbox, sensor_device_health for device battery/signal/firmware tracking), 2 views (v_data_freshness_summary for current freshness status, v_sensor_device_health_summary for device health overview), `006_telemetry_sync.sql` adding 17 missing columns to ClickHouse remote_sensing_events (msavi, satvi, bsi, nbr2, ndti, lswi, brightness_index, tasseled cap, raw bands, source_system) and mv_daily_remote_sensing_summary materialized view, `services/ingestion/data_freshness.py` (freshness checker with webhook/email alerting, configurable thresholds, 7 default SLAs), `services/ingestion/climate_data.py` (WorldClim/NCEP/MODIS/SMAP/Sentinel-1 fetchers with latitude-based bioclimatic estimation, dual-write to 5 climate tables), updated `services/ingestion/remote_sensing.py` dual-write to include all 17 new ClickHouse columns, `docs/telemetry-infrastructure.md`, 2 test files, cron entries for hourly freshness checks and weekly climate refresh. 30+ unit tests.
- **Remote Sensing Automation (GEE + Copernicus)**: Added `services/ingestion/remote_sensing_fetcher.py` (abstract orchestrator with job scheduling, bbox resolution, provider dispatch, status tracking), `services/ingestion/gee_remote_sensing.py` (Google Earth Engine adapter using Sentinel-2 SR Harmonized, server-side cloud-free median composites, 15+ spectral index computation including NDVI, NDRE, EVI, SAVI, MSAVI, NDWI, BSI, SATVI, NBR2, NDTI, LSWI, brightness index, tasseled cap, raw bands, dual-write to PG + CH), `services/ingestion/copernicus_remote_sensing.py` (Copernicus Data Space adapter using OData catalog API, OAuth2 client credentials, product metadata ingestion), `tests/test_remote_sensing_automation.py` (30+ tests for index computation, provider integration, schema validation). Added `earthengine-api>=1.2.0` to requirements.txt. Remote sensing fetcher CLI with --run-jobs, --list-jobs, --job-id, --location-id, --provider flags.
- **IoT Sensor Push (MQTT + HTTP + Device Manager)**: Added `services/ingestion/mqtt_subscriber.py` (paho-mqtt subscriber for real-time sensor readings, topic-based routing for sensors/{location}/{type}/readings and sensors/{location}/{type}/register, auto-device registration, dual-write to PG + CH, ingestion logging, graceful shutdown), `services/ingestion/http_sensor_receiver.py` (FastAPI webhook receiver with POST /api/v1/sensors/{id}/readings and /batch endpoints, HMAC signature verification, health check endpoint, dual-write to PG + CH), `services/ingestion/device_manager.py` (device registration with ON CONFLICT update, device listing with connectivity status, health tracking with battery/signal/firmware/reading rate, auto-init of sensor_device_health), `config/mosquitto/mosquitto.conf` (MQTT broker config with persistence, logging, message limits), `docker-compose.yml` updated with mosquitto service (eclipse-mosquitto:2 on databases network, port 1883, health check). Added paho-mqtt>=2.0.0, fastapi>=0.100.0, uvicorn>=0.20.0 to requirements.txt. `tests/test_iot_sensor_push.py` (30+ tests). MQTT subscriber listens on sensors/+/+/readings and sensors/+/+/register topics. HTTP receiver available at 0.0.0.0:8056.
- **Tokenized Carbon Credits with Auto-Adjustment**: Added `078_carbon_credits.sql` with 4 new governed tables (carbon_credit for credit identity, vintage, methodology, sequestration quantities with buffer pool, configurable pricing, Level 6 evidence requirement, and EAS attestation linkage; credit_adjustment for auto-adjustment audit trail with trigger source, delta tracking, margin check, and review queue; credit_retirement for permanent retirement ledger with on-chain attestation; credit_transfer for allocation/sale/grant ledger with financial linkage to revenue events), 3 public views (v_public_carbon_credit_inventory gated on Level 6 evidence and published farm_registry_record, v_carbon_credit_balance for per-location summary, v_credit_adjustment_history for adjustment audit trail), `services/analytics/carbon_credits.py` with issue_credit (auto-generates KKNT-{year}-{location}-{seq} code, applies buffer pool, links to climate_impact_summary), adjust_credit (recomputes sequestration from live data, auto-adjusts within margin, queues review for reversals/exceedances), retire_credit (permanent retirement with balance check), list_credits, get_balance, check_adjustments. Configurable buffer pool percentage per location, configurable pricing (default + market-adjusted), auto-adjustment margin with human review for reversals. 30+ unit tests. `carbon_credit`, `credit_adjustment`, `credit_retirement`, `credit_transfer` added to GOVERNED_COLLECTIONS in agents/safety.py.
- **ML Anomaly Detection (Prophet + Isolation Forest)**: Added `services/ingestion/ml_anomaly_detector.py` with Facebook Prophet for univariate seasonal anomaly detection (daily + weekly seasonality, prediction intervals as anomaly thresholds, per-sensor per-location models) and scikit-learn Isolation Forest for multivariate cross-sensor anomaly detection (multi-sensor feature matrix, time features, lagged features, contamination-based anomaly scoring). Lazy imports with graceful fallback when ML dependencies unavailable. Alert fusion merges Prophet and IF anomalies, deduplicates, writes to sensor_alert with detection_method field. Model persistence with pickle serialization to models/ directory. CLI: --ml-check (run detection), --ml-train (fit and save models), --location-id, --sensor. Updated anomaly_detector.py with --ml-check and --ml-train flags. Added pandas>=2.0, numpy>=1.24, prophet>=1.1.5, scikit-learn>=1.3 to requirements.txt. 20+ unit tests.
- **Prefect Workflow Engine**: Added `services/flows/` package with 15 Prefect flow definitions wrapping existing CLI tools (weather, sensors, market_data, eas_indexer, rpc_indexer, gnosis_indexer, remote_sensing, anomaly_detection, metrics, dashboards, freshness, climate_data, credit_adjustment, health_check), `services/flows/pipelines.py` with 8 pipeline definitions (ingestion_pipeline, monitoring_pipeline, analytics_pipeline, full_pipeline, scheduled_hourly, scheduled_every_6h, scheduled_daily, scheduled_weekly) with explicit dependency chains (ingestion->monitoring->analytics), `config/prefect/deployment.yaml` with 5 scheduled deployments (hourly, every-6h, daily, weekly, manual full). Added prefect>=3.0 to requirements.txt. Each flow uses retries and retry_delay_seconds. 15+ unit tests.
- **Field Data Collection Guide + Water Sampling**: Added `080_field_collection.sql` with water_sample table (field-level water sampling that feeds into water_analysis lab table, with collection method, depth, container type, preservative, chain of custody, GPS, field measurements, lab linkage), governed lifecycle additions to soil_sample, soil_carbon_measurement, and species_observation tables (status, verified_by, verified_at, schema_version, evidence_maturity), public water sample view. Added `docs/field-data-collection-guide.md` — detailed SOP covering 14 sections: equipment catalog (7 tool categories with specifications), soil sampling protocol (depth, composite, GPS, lab submission), tree measurement protocol (DBH at 1.3m, height estimation methods, canopy, health scoring 0-100), biodiversity survey protocol (quadrat, transect, point count, camera trap, pitfall trap, timing by taxa), water sampling protocol (collection, field measurements, preservation, chain of custody, lab submission), weather observation (on-site station placement, manual readings, API reconciliation), harvest measurement (weighing, grading A/B/C/rejected, loss measurement, post-harvest handling), pest monitoring (visual scouting, sticky/pheromone traps, soil coring, disease scouting), measurement cadence matrix (what/when/who/how for 18 data types), data quality & QA/QC (range validation, duplicate detection, observer calibration, evidence maturity escalation), sample plot design (plot types, layout, establishment, re-measurement, permanence rules), MRV ground-truthing (field-to-digital pipeline, reconciliation with remote sensing, community MRV), equipment maintenance (calibration schedules, battery management, cleaning, replacement). Updated telemetry-infrastructure.md with field collection section.
- **Demand Analysis & Demand Sizing**: Added `069_demand_analysis.sql` with 6 new governed tables (buyer_demand_signal for buyer intent and future orders, demand_forecast for predicted demand by crop/period, market_size_estimate for TAM/SAM/SOM calculations, demand_trend for seasonal patterns and price elasticity, production_market_match for supply-demand gap analysis, buyer_segment for behavioral segmentation), 6 public views, 7 new metrics (demand_coverage_pct, buyer_pipeline_value, market_penetration_pct, demand_supply_gap_tonnes, buyer_retention_rate, demand_seasonality_index, buyer_diversification_score), `services.analytics.demand` (5 functions: compute_demand_forecast, compute_market_sizing, compute_demand_trends, compute_buyer_segmentation, compute_production_market_match), Adelphi pilot data (4 buyer demand signals, 2 market size estimates), and 12 tests.
- **Impact Network: 5-Component Expansion**: Added 5 schema files (064-068) implementing Organization entity, Funding Flow, Ecosystem Landscape, Impact Bounties, and Impact Office orchestrator. 064_organization.sql: organization, organization_member, organization_wallet tables with FK additions to location/partner/staff. 065_funding_flow.sql: donor, funding_campaign, donation, impact_payout_rule, impact_payout_execution tables for crowdfunding and impact-linked payouts. 066_ecosystem_landscape.sql: v_ecosystem_farm_network, v_ecosystem_farm_detail, v_farm_funding_page views for farm network visualization and Fund Farm CTA. 067_impact_bounties.sql: impact_bounty and impact_bounty_submission tables for incentivized data collection. 068_impact_office.sql: impact_office_run, impact_office_step, impact_office_alert tables for unified orchestration. services/office/ with orchestrator.py and cli.py. 13 tests.
- **Statement of Work Production**: Added `063_statement_of_work.sql` with 4 new governed tables (statement_of_work for document metadata, sow_deliverable with acceptance criteria, sow_payment_schedule for payment milestones, sow_change_request for change management), 3 public views (v_public_statement_of_work, v_public_sow_deliverables, v_public_sow_payment_schedule), `generate_statement_of_work()` report generator that assembles SOW from 12 existing tables (objective, farm_task, development_phase, framework_step, financial_sustainability_plan, forecast_output, risk_mitigation_register, staff, department, job_role, partner, location), Adelphi pilot data (1 SOW, 3 deliverables, 3 payment milestones, 1 change request), and 10 tests. Report type accessible via `python3 -m services.export.report_generator --type statement_of_work --location-id UUID`.
- **Metrics by Development Phase documentation**: Added `docs/metrics-by-phase.md` mapping all 62 schema phases to their enabled metrics, individual question answers, and collective cross-phase insights across 6 tiers (Foundation, Governance, Frameworks, Wellbeing, Operations, True Cost). Updated README.md with "How Metrics Enable Answers" section and reference to new doc. Updated AGENTS.md with new agent, test, and report commands. Updated data dictionary and architecture docs with new table sections.
- **True Cost Accounting & Triple Bottom Line Integration**: Added `062_true_cost_accounting.sql` with 9 new governed tables across 5 phases: Phase 1 (hidden_cost_observation for externalities with monetary estimates, natural_capital_valuation for unified carbon/biodiversity/water/soil/pollination valuation), Phase 2 (social_impact_valuation for training/governance/cultural/health/community monetization, worker_safety_observation for workplace incidents, living_wage_benchmark for wage comparison), Phase 3 (lca_assessment for cradle-to-grave lifecycle tracking), Phase 4 (gri_indicator for GRI standards mapping, materiality_assessment for stakeholder priority), Phase 5 (capital_flow_observation for cross-capital transfers). 4 public views (v_public_hidden_costs, v_public_natural_capital_valuation, v_public_social_impact_valuation, v_true_cost_statement combining market costs + hidden costs + capital values). 4 analytics modules: true_cost_accounting.py (5 functions: compute_true_cost_statement, compute_hidden_cost_summary, compute_natural_capital_valuation, compute_social_impact_valuation, compute_human_capital_score), life_cycle.py (3 functions: compute_product_carbon_footprint, compute_water_footprint, compute_lca_summary), gri_reporting.py (2 functions: compute_gri_compliance_score, compute_materiality_matrix), systems_thinking.py (3 functions: compute_capital_flow_summary, compute_cross_capital_dependencies, compute_system_resilience). Adelphi pilot data across all 9 tables. 30 tests.
- **Impact Value Chain Parity**: Added `061_impact_value_chain.sql` with 6 new governed tables (department, job_role, farm_task, weekly_plan, development_phase, framework_step), staff table extension (job_role_id, department_id, hire_date, employment_status FKs), 4 public views (v_public_farm_tasks, v_public_weekly_plans, v_public_development_phases, v_public_framework_steps), `services.analytics.impact_value_chain` (4 functions: compute_task_completion_rate, compute_weekly_plan_adherence, compute_development_phase_progress, compute_framework_step_progress with prerequisite validation), Adelphi pilot data (5 departments, 7 job roles, 5 farm tasks, 2 weekly plans, 3 development phases, 6 framework steps, staff FKs linked), and 16 tests.
- **Scenario Parameters: Worst/Base/Best Case Modeling**: Added `060_scenario_parameters.sql` with 2 new governed tables (scenario_parameter for worst/base/best case definitions with distribution types, scenario_simulation for Monte Carlo/sensitivity/stress test results), 2 public views (v_public_scenario_parameters, v_public_simulation_results), `services.forecast.scenario_params` (load, sample, apply parameters), `services.forecast.monte_carlo` (3 functions: run_monte_carlo with 1000-sample simulation, run_sensitivity_analysis per parameter, run_sensitivity_tornado for ranked impact), 18 default parameters across 6 categories (yield: loss_rate, drought_yield_impact, pest_disease_loss, yield_improvement; price: maize/cassava/beans/sweet_potato; cost: fertilizer/labor/transport/seed inflation; weather: drought/flood probability; ecological: carbon_sequestration_rate, biodiversity_credit_price; governance: public_goods_allocation_pct, retention_rate_pct), Adelphi pilot data for all 3 scenarios (baseline, optimistic, conservative), and 23 tests.
- **Drone & Raster Integration (Silvi Phase 3)**: Added `059_drone_raster_integration.sql` with MSAVI column on remote_sensing_observation, 3 new governed tables (raster_metadata for GeoTIFF/orthomosaic references, spatial_cluster for DBSCAN results, pest_hotspot for spatial pest clustering), 5 public views (v_public_spatial_clusters, v_public_pest_hotspots, v_public_canopy_analysis, v_zone_adjacency, v_habitat_connectivity_summary), `services.analytics.spatial_analytics` (5 functions: compute_spatial_clusters, compute_pest_hotspots, compute_canopy_analysis, compute_gap_detection, compute_habitat_connectivity with 0-100 connectivity score), Adelphi pilot data (2 raster metadata, 3 spatial clusters, 1 pest hotspot), and 20 tests.
- **Spatial Export (Silvi Phase 2)**: Added `058_spatial_export.sql` with geometry column on farm_zone, 6 public views (v_spatial_zone_geojson, v_spatial_tree_geojson, v_spatial_location_geojson, v_spatial_plot_geojson, v_spatial_buffer_geojson, v_spatial_project_summary), `services.export.spatial_export` (6 export functions: export_zone_geojson, export_tree_geojson, export_location_boundary_geojson, export_project_geojson, export_zone_kml, export_project_xml), Adelphi zone geometries (4 zones with PostGIS polygons), and 11 tests.
- **Individual Tree Tracking (Silvi Phase 1)**: Added `057_tree_tracking.sql` with 2 new governed tables (tree_record with GPS coordinates and PostGIS point_geometry, tree_measurement for time-series growth tracking), 2 public views (v_public_tree_records, v_public_tree_density_map), 1 new metric (tree_density_per_ha), auto-computed PostGIS geometry trigger, unique tree_tag per plot constraint, `services.analytics.tree_tracking` (3 functions: compute_tree_density, compute_growth_rate, compute_species_richness with Shannon and Simpson indices), Adelphi pilot data (20 individual trees with GPS coordinates across 3 plots, 15 tree measurements simulating 3 quarterly surveys), and 17 tests.
- **Emergency Response System**: Added `056_emergency_response.sql` with 1 new governed table (emergency_incident), 1 public view (v_public_emergency_incidents), 1 new metric (emergency_response_time_days), `services.analytics.emergency_response` (2 functions: compute_emergency_response_time, compute_emergency_impact_summary), Adelphi pilot data (drought incident, high severity, 21-day resolution, response actions, recovery, lessons learned), and 15 tests.
- **Organic Certification Readiness & Compliance**: Added `055_organic_certification.sql` with 8 new governed tables (organic_certification_record, organic_transition_plan, prohibited_substance_record, buffer_zone, organic_input_audit, harvest_handling_record, organic_compliance_checklist, organic_readiness_assessment), 8 public views (v_public_organic_readiness, v_public_organic_transition, v_public_organic_certifications, v_public_prohibited_substance_audit, v_public_buffer_zone_status, v_public_organic_input_audit, v_public_harvest_segregation, v_public_organic_compliance_dashboard), 2 new metrics (organic_readiness_score, organic_input_compliance_pct), `services.analytics.organic_certification` (7 functions: compute_organic_readiness_score, compute_transition_progress, compute_input_compliance_pct, compute_buffer_adequacy, compute_harvest_segregation_score, compute_record_completeness, compute_prohibited_substance_clearance), `services.agents.organic_readiness_agent`, 3 report types (organic_certification_readiness, organic_transition_progress, organic_input_audit), Adelphi pilot data (1 IFOAM certification record, 1 transition plan year 2/3, 2 buffer zones, 3 organic input audits, 2 harvest handling records, 1 compliance checklist 14/18 pass, 1 readiness assessment 72.5/100, 1 EAS compliance attestation), and 20 tests.
- **Ecological Modeling Hub v2**: Added `047_ecological_modeling_v2.sql` with 4 new governed tables (soil_input_application, pest_observation, biocontrol_release, resource_consumption), conservation_status extension to species_observation, 4 public views, 4 new metrics (pest_outbreak_probability, biocontrol_effectiveness_pct, labor_efficiency_kg_per_hour, resource_intensity_index), 2 dashboards (64-65), Adelphi pilot data (soil inputs, pest observations, biocontrol releases, resource consumption, pest dynamics model run), `services.analytics.ecological_modeling_v2`, `services.analytics.resource_efficiency`, 2 report types (49 total), docs, and tests.
- **Economic and Social Enhancement**: Added `048_economic_social_enhancement.sql` with 2 new governed tables (training_session, revenue_stream_contribution), 2 public views, 2 new metrics (training_improvement_pct, revenue_per_acre_usd), Adelphi pilot data (4 training sessions, 4 revenue stream contributions), `services.analytics.economic_performance`, 2 report types (training_impact, revenue_streams), and tests.
- **Model Validation**: Added `049_model_validation.sql` with 2 new governed tables (prediction_accuracy_record, feature_importance_record), 2 public views, 2 new metrics (forecast_mae, forecast_accuracy_pct), Adelphi pilot data (3 prediction accuracy records, 5 feature importance records), `services.analytics.model_validation`, 1 report type (model_validation), and tests.
- **Remaining Gaps Closure**: Added `050_remaining_gaps.sql` with 6 new governed tables (leaf_litter_measurement, livestock_group, feed_intake_record, decomposition_measurement, token_reward_distribution, reward_calibration_model), predation rate extension to pest_observation, auto-computed decomposition_rate_kg_per_day and mass_loss_pct, 5 public views, 6 new metrics (leaf_litter_rate_kg_per_day, feed_conversion_ratio, decomposition_rate_pct_per_day, predation_rate_per_day, token_rewards_per_epoch, reward_calibration_score), 2 dashboards (66-67), `services.analytics.livestock_feed`, `services.analytics.reward_calibration`, 3 report types (52 total), and tests.
- **Gap Closures for 20-Question Verification**: Added `051_gap_closures.sql` with irrigation_mm_used and rainfall_mm_during_period extensions to resource_consumption, 3 public views (v_public_rainfall_vs_irrigation, v_public_species_richness_per_ha, v_public_location_species_richness), 3 new metrics (yield_delta_kg, species_richness_per_ha, rainfall_irrigation_delta_mm), `services.analytics.gap_closures` (4 functions: compute_yield_delta, compute_soil_nutrient_input_correlation, compute_species_richness_per_ha, compute_rainfall_vs_irrigation), soil input retention data added to ecological_modeling report, and tests.
- **Configurable Container Architecture**: Added `053_configurable_containers.sql` with 5 new governed tables (farm_template, farm_specification, needs_assessment, stakeholder_aspiration, objective), 5 public views, 2 new metrics (objective_completion_pct, needs_resolution_rate), `services.analytics.configurable_containers` (5 functions: compute_farm_template_summary, compute_farm_specification_status, compute_needs_assessment_summary, compute_aspirations_summary, compute_objectives_progress), Adelphi pilot data (syntropic template, farm specification, 3 needs assessments, 3 aspirations, 5 hierarchical objectives), and tests.
- **Grant Application & Network Diversity**: Added `054_grant_network.sql` with 3 new governed tables (grant_application_history, regional_chapter, network_membership), returning_applicant/grant_count fields on farm_registry_record, 4 public views (v_public_grant_application_history, v_public_regional_chapters, v_network_diversity, v_public_farm_network_summary), 2 new metrics (grant_funding_rate, network_species_diversity), `services.analytics.grant_network` (3 functions: compute_grant_history_summary, compute_network_diversity_summary, compute_regional_chapter_detail), Adelphi pilot data (2 grant applications, 1 regional chapter, 1 network membership), Grant Application Template farm_template, and tests.
- **Farm Registry People category**: Added `founders JSONB` column to `farm_registry_record` for structured founder identity, public figures, and leadership. Updated DAOIP-5 view to expose founders in project JSON. Adelphi pilot seed includes founder entry. Schema version remains `common-data-schema-v1`.
